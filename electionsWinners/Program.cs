...
// equal to the number of voters who haven't cast their vote yet, find the number of candidates
...
// RESULT: Thus, only 2 candidates can win(the second and the third), which is the answer.
...
namespace electionsWinners
{
    class Program
    {
        ...
        {
            // Initializing test values and print the test result
            int[] test = new int[] { 2, 3, 5, 2 , 4};
            int k = 3;
            Console.WriteLine(electionsWinners(test,k));
            Console.ReadKey();

        }

        // The method returns the number of possible winners in elections
        static int electionsWinners(int[] votes, int k)
        {
            int count = 0; // will be the number of possible winners
            int maxCounts = 0; // will be the number of current leaders, who have equally maximum votes
            int maxVotes = votes.Max(); // current maximum votes of a candidate

            // calculates who can beat the maxVotes
            ...
            if (k == 0 && maxCounts == 1) count = 1; // if the elections were ended, and there was only one with maxVotes
            return count;
        }
    }
}
